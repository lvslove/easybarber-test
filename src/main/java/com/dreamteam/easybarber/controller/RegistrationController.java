        ...
        if(user.getCity() != null && user.getAccounttype() != null) {
            ...
            if (!user.getEmail().isEmpty()) {
                String message = String.format(
                        "Добрый день, %s! \n" +
                                "Добро пожаловать в дружную команду \"Easy barber\". Пожалуйста, перейдите по ссылке для подтверждения аккаунта: %s/activate/%s",
                        user.getUsername(),
                        path,
    ...
    public String checkPhone(@RequestParam String phonenumber, Model model)
    {
        ...
        if(user != null)
        {
            ...
            if (!user.getEmail().isEmpty()) {
                String message = String.format(
                        "Добрый день, %s! \n" +
                                "перейдите по ссылке для подтверждения смены пароля: %s/reboot/%s \n" +
